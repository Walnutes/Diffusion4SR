...
def resize_and_convert(img, size, resample):
    ...
    w,h = img.size
    # if w>=h:
    #     im_new = Image.new(mode='RGB', size=(w,w), color=255)
    #     im_new.paste(img, box=(0, (w-h)//2))
    # else:
    #     im_new = Image.new(mode='RGB', size=(h,h), color=255)
...
def prepare(img_path, out_path, n_worker, sizes=(128, 32), resample=Image.BICUBIC, lmdb_save=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size', type=str, default='(128,32)/(512,128)')
    ...
    sizes = [eval(s.strip()) for s in args.size.split('/')]
    # Edition 
