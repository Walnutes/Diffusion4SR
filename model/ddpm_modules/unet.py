...
class UNet(nn.Module):
    def __init__(
        ...
        for ind in range(num_mults):
            is_last = (ind == num_mults - 1)    # num_mults = len(channel_mults) 
                                                # channel_mults=(1, 2, 4, 8, 8),
                                                # is_last判断是否为最后一个
                                            
            use_attn = (now_res in attn_res)    # TODO:attn_res=(16),目前似乎并没有使用
    
            channel_mult = inner_channel * channel_mults[ind] # channel_mults应该是一个系数序列，如(1, 2, 4, 8, 8)
            ...
            for _ in range(0, res_blocks):
                ...
                pre_channel = channel_mult   # 将目前通道数添加进入序列，同时更新pre_channel的数值
                
            if not is_last:
                downs.append(Downsample(pre_channel))
                feat_channels.append(pre_channel)
                
                # 若非最后一个模型的block，则需要进行下采样，但下采样并不改变通道数，因此只相加到feat_channels即可
                ...
                """
                tup = eval(now_res)
                new_tup = (tup[0]//2,tup[1]//2)
                now_res = '(' + str(tup[0]//2) +',' + str(tup[1]//2)+ ')'
                """
        ...
        for ind in reversed(range(num_mults)):
            ...
            # 妙：新的判断是否为有序序列最后一项的方法，使用reversed函数倒置。
